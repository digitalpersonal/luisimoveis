...
const Chat: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden animate-in fade-in duration-500">
      {/* Sidebar List */}
      ...
      <div className="flex-1 flex flex-col bg-slate-50/50 relative">
        <div className="px-6 py-4 bg-white border-b border-slate-100 flex items-center justify-between sticky top-0 z-10 shadow-sm">
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center font-black">
              {selectedChat.name.charAt(0)}
            </div>
            <div>
              <h4 className="text-sm font-black text-slate-900">{selectedChat.name}</h4>
              <p className="text-[10px] text-emerald-500 font-black uppercase tracking-[0.15em] flex items-center gap-1">
                <Circle size={8} fill="currentColor" /> {selectedChat.online ? 'Online agora' : 'Offline'}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-1.5 text-slate-400">
            <button onClick={() => handleMediaAction('phone')} className="p-2.5 hover:bg-slate-50 rounded-xl hover:text-indigo-600 active:scale-90 transition-all" title="Chamada"><Phone size={18} /></button>
            <button onClick={() => handleMediaAction('video')} className="p-2.5 hover:bg-slate-50 rounded-xl hover:text-indigo-600 active:scale-90 transition-all" title="Vídeo"><Video size={18} /></button>
            <button onClick={() => handleMediaAction('more')} className="p-2.5 hover:bg-slate-50 rounded-xl hover:text-indigo-600 active:scale-90 transition-all" title="Mais"><MoreVertical size={18} /></button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
          ...
          {chatHistory.map((msg) => (
            <div key={msg.id} className={`flex gap-4 animate-in fade-in slide-in-from-bottom-2 duration-300 ${msg.sender === 'admin' ? 'flex-row-reverse' : ''}`}>
              <div className={`w-9 h-9 rounded-xl shrink-0 flex items-center justify-center text-xs font-black shadow-sm ${msg.sender === 'admin' ? 'bg-indigo-600 text-white' : 'bg-white text-indigo-600 border border-slate-100'}`}>
        ...
        <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-slate-100 flex items-center gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)]">
          <button type="button" onClick={() => handleMediaAction('attach')} className="p-2.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-xl active:scale-90 transition-all" title="Anexar"><Paperclip size={20} /></button>
          <div className="flex-1 relative">
            <input 
              ...
              className="w-full pl-4 pr-12 py-3 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all"
            />
          ...
          <button 
            ...
            className="p-3.5 bg-indigo-600 text-white rounded-2xl shadow-xl shadow-indigo-100 hover:bg-slate-900 transition-all active:scale-95 disabled:opacity-50 group"
          >
